...
#
# 2025/02/13 ImageMaskDatasetGenerator.py
...
class ImageMaskDatasetGenerator:
  ...
  def __init__(self, 
               input_dir  = "./ImageALCAPA_BIBM_Publish/", 
               output_dir = "./ALCAPA-master", 
               angle      = 0, 
               resize     = 512,
               augmentation=True):
    ...
    self.image_file = "image.nii.gz"
    self.seg_file   = "label.nii.gz"
    print("--- image_file {} seg_file {}".format(self.image_file, self.seg_file))
  ...
  def generate(self):
    index = 100000
    seg_files   = glob.glob(self.input_dir + "/*"  + self.seg_file)
    image_files = glob.glob(self.input_dir + "/*"  + self.image_file)
    l1 = len(seg_files)
    l2 = len(image_files)
    ...
    for i in range(l1):
      index += 1000
      self.generate_mask_files(seg_files[i],   index) 
